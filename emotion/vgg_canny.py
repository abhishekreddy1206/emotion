...
# -*- coding: utf-8 -*-
"""
VGG-like architecture with Canny edge detection preprocessing.
...
def custom_img_prep_samp(img):
    ...
    figure, (img1, img2, img3, img4) = pyplot.subplots(nrows=1, ncols=4, figsize=(8, 3),
...
def data_gen(data_dir):
    ...
    data_train = datagen.flow_from_directory(directory=data_dir + 'train', target_size=(224, 224))
    data_test = datagen.flow_from_directory(directory=data_dir + 'validation', target_size=(224, 224))
    return datagen, data_train, data_test


def data_gen_samp(datagen, data_dir):
    samp_img = load_img(data_dir + '/train/happy/S057_006_00000033.png')
    samp_data_itr = datagen.flow(np.expand_dims(img_to_array(samp_img), 0), batch_size=1)
...
datagen, data_train, data_test = data_gen(data_dir)
data_gen_samp(datagen, data_dir)
